...
```jsonc
{
    "method": "pay_invoice",
    "params": {
        "invoice": "lnbc50n1...", // bolt11 invoice, or bolt12 offer if make_offer is supported
...
```jsonc
{
    ...
    "params": {
        "invoices": [
          {"id":"4da52c32a1", "invoice": "lnbc1...", "amount": 123}, // bolt11 invoice, or bolt12 offer if make_offer is supported and amount in msats, amount is optional
...
```jsonc
{
    "method": "make_offer",
    "params": {
        "amount": 123, // optional value in msats, no amount means offer supports any amount
        "description": "string", // offer's description, optional
        "expiry": 213, // expiry in seconds from time offer is created, optional
        "issuer": "string" // public information about the offer issuer, optional
    }
}
```
...
```jsonc
{
    ...
    "result": {
        "type": "incoming", // "incoming" for offers
        "offer": "string", // encoded offer
        "description": "string", // offer's description, optional
        "amount": 123, // value in msats, optional
        "issuer": "string" // public information about the offer issuer, optional
        "expires_at": unixtimestamp, // offer expiration time, optional if not applicable
        "metadata": {} // generic metadata that can be used to add things like zap/boostagram details for a payer name/comment/etc.
    }
}
```
...
```jsonc
{
    "method": "lookup_offer",
    "params": {
        "offer": "lnbc50n1..." // bolt12 offer to lookup
    }
}
```
...
```jsonc
{
    ...
    "result": {
        "type": "incoming", // "incoming" for normal offers
        "offer": "string", // encoded bolt12 offer, optional
        "description": "string", // bolt12 offer's description, optional
        "amount": 123, // value in msats, optional
        "issuer": "string" // public information about the offer issuer, optional
        "expires_at": unixtimestamp, // bolt12 offer expiration time, optional if not applicable
        "metadata": {} // generic metadata that can be used to add things like zap/boostagram details for a payer name/comment/etc.
    }
}
```
